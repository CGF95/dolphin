...
void TASInputDlg::CreateWiiLayout(int num)
{
  ...
  auto* const m_buttons_box = new wxStaticBoxSizer(wxVERTICAL, this, _("Buttons"));
...
wxStaticBoxSizer* TASInputDlg::CreateStickLayout(Stick* tempStick, const wxString& title)
{
  ...
  layout->Add(tempStick->x_cont.slider, 0, wxEXPAND);
  layout->Add(tempStick->x_cont.text, 0, wxALIGN_CENTER);
  layout->Add(tempStick->bitmap, 0, wxALIGN_RIGHT);
  layout->Add(tempStick->y_cont.slider, 0, wxEXPAND);
  layout->AddSpacer(1);  // Placeholder for unused cell
  layout->Add(tempStick->y_cont.text, 0, wxALIGN_CENTER);
...
void TASInputDlg::UpdateFromInvalidatedButton(wxCommandEvent& event)
{
  ...
  //ASSERT_MSG(PAD, button->id == button->checkbox->GetId(), "Button ids do not match: %i != %i", button->id, button->checkbox->GetId());
  button->checkbox->SetValue(button->value);
}
...
void TASInputDlg::UpdateFromInvalidatedControl(wxCommandEvent& event)
{
  ...
  //ASSERT_MSG(PAD, control->text_id == control->text->GetId(), "Control ids do not match: %i != %i", control->text_id, control->text->GetId());
  control->text->SetValue(std::to_string(control->value));
}
...
void TASInputDlg::ExecuteHelpers()
{
  ...
  else
  {
	...
	if (m_quickspin.checkbox->IsChecked())
	{
		...
		quickspin_timer = Movie::GetCurrentFrame();
	...
	if (m_rollassist.checkbox->IsChecked() && !Movie::roll_enabled)
	{
		...
		{
			Movie::roll_enabled = true;
			Movie::roll_timer = Movie::GetCurrentFrame();
		}
	...
	if (Movie::roll_enabled)
	{
		...
		if (isTP)
		{
			...
			if (Movie::first_roll)
			{
				...
				else if (Movie::GetCurrentFrame() == Movie::roll_timer + 23) //22 would be ideal, but terrain...
				{
					m_a.checkbox->SetValue(true);
				}
				else if (Movie::GetCurrentFrame() == Movie::roll_timer + 24)
				{
					m_a.checkbox->SetValue(false);
					Movie::first_roll = false;
					Movie::roll_timer = Movie::GetCurrentFrame() - 1;
			...
			{
				if (Movie::GetCurrentFrame() == Movie::roll_timer + 20) //19 would be ideal, but terrain...
				{
					m_a.checkbox->SetValue(true);
				}
				else if (Movie::GetCurrentFrame() == Movie::roll_timer + 21)
				{
					m_a.checkbox->SetValue(false);
					Movie::roll_timer = Movie::GetCurrentFrame() - 1;
				}
		...
		if (isTWW)
		{
			...
			if (characterpointer > 0x80000000)
			{
				...
				else
				{
					...
					{
						m_a.checkbox->SetValue(false);
						Movie::roll_timer = Movie::GetCurrentFrame() + 15; //16 to 15
					}
				}
			}
			else
