...
int GBASockServer::CreateFakeResponse(u8* si_buffer)
{
	...
	*/
	...
	if (frameTarget != 0)
	{
	if (frameTarget <= Movie::GetCurrentFrame())
	{
	std::string output;
	...
	*/
	...
	if (Movie::tunerExecuteID > 0 && Movie::tunerExecuteID < 18 || Movie::tunerExecuteID > 19)
	{
		...
		{
			std::string output;
			output = StringFromFormat("\n%d: ACTION ID: %i\n", Movie::GetCurrentFrame(), Movie::tunerExecuteID);
		...
		else if (actionPhase == 0)
		{
			...
			else
			{
				...
				if (send_data[4] == 0x30)
				{
					if (tingleRNG)
					{
						if (frameTarget <= Movie::GetCurrentFrame()) //Timeout
						{
							tingleRNG = false;
							frameTarget = 0;
					...
					else if (!tingleRNG)
					{
						...
						if (!gameID.compare("GZLJ01"))
						{
							...
							if (currLoading > 0)
							{
								inLoading = true;
								actionPhase = 2;
								frameTarget = Movie::GetCurrentFrame() - 1;
							}
							else
							{
								if (inLoading)
								{
									if (eventFlag == 0)
									{
										if (Movie::GetCurrentFrame() > frameTarget)
										{
											inLoading = false;
									...
									{
										frameTarget = Movie::GetCurrentFrame() + 30;
									}
								}
							}
						}
					...
					if (Movie::tunerExecuteID > 9 && Movie::tunerExecuteID < 18 || Movie::tunerExecuteID > 19 && Movie::tunerExecuteID < 25)  //Bomb = 10, Balloon = 11, Shield = 12, Kooloolimpah = 13, Red = 14, Green = 15, Blue = 16, Discount Blue = 20, Shield+Balloon = 21, Extended Shield = 22, Nothing = 23, Nothing (Load) = 24
					{
						...
						else if (localConnectionPhase == 13)
						{
							...
							else if (Movie::tunerExecuteID == 13) //Kooloolimpah
							{
								...
								//frameTarget = Movie::GetCurrentFrame() + 90;
		...
		else if (actionPhase == 4)
		{
			...
			else
			{
				...
				else if (localConnectionPhase == 8)
				{
					...
					{
						tingleRNG = true;
						frameTarget = Movie::GetCurrentFrame() + 150;
					}
					else
					{
						Movie::tunerExecuteID = 0;
...
int CSIDevice_GBA::RunBuffer(u8* buffer, int length)
{
	...
	if (isEnabled) //Fake GBA
	{
		...
		if (!isConnected)
		{
			...
			{
				num_data_received = CreateFakeResponse(buffer);
			}
	...
	else //Real GBA
	{
		...
		if (waiting_for_response && num_data_received == 0)
		{
			num_data_received = Receive(buffer);
		}
